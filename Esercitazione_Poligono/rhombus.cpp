...
float rhombus::Perimeter() {
	return Side(diagH,diagV) * 4;
}

void rhombus::Draw() {
	cout << "This is a rhombus with this features:" << endl << "1) horizontal diagonal = " << diagH << endl << "2) vertical diagonal = " << diagV << endl
		<< "3) side = "<<Side(diagH,diagV)<<endl<<"4) perimeter = " << Perimeter() << endl << "5) area = " << Area() << endl << endl << endl;
}